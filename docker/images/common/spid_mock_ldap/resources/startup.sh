...
EOF
  ...
  ldapadd -x -D "cn=admin,dc=healthgateway,dc=it" -w hgwpwd -f /mickeymouse.ldif > /dev/null
  ldappasswd -x -D "cn=admin,dc=healthgateway,dc=it" -s mouse "uid=mouse,dc=healthgateway,dc=it" -w hgwpwd
  ldapadd -x -D "cn=admin,dc=healthgateway,dc=it" -w hgwpwd -f /donaldduck.ldif > /dev/null
  ldappasswd -x -D "cn=admin,dc=healthgateway,dc=it" -s duck "uid=duck,dc=healthgateway,dc=it" -w hgwpwd
...
# ldapsearch -x -H ldap://localhost -b uid=duck,dc=healthgateway,dc=it -D "cn=admin,dc=healthgateway,dc=it" -w healthgatewaypwd > /dev/null
# if [ ! $? -eq 0 ]; then
# 	ldapadd -x -D "cn=admin,dc=healthgateway,dc=it" -w healthgatewaypwd -f /mickeymouse.ldif > /dev/null
# 	ldapadd -x -D "cn=admin,dc=healthgateway,dc=it" -w healthgatewaypwd -f /donaldduck.ldif > /dev/null
# fi
