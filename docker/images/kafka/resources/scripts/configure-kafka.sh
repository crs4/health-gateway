...
if [[ -z "$KAFKA_LISTENERS" ]]; then
    export KAFKA_LISTENERS=PLAINTEXT://0.0.0.0:$KAFKA_PORT,SSL://0.0.0.0:$KAFKA_SSL_PORT
    # unset KAFKA_PORT
fi
if [[ -z "$KAFKA_ADVERTISED_LISTENERS" ]]; then
    export KAFKA_ADVERTISED_LISTENERS="PLAINTEXT://$KAFKA_ADVERTISED_HOST_NAME:$KAFKA_ADVERTISED_PORT,SSL://$KAFKA_ADVERTISED_HOST_NAME:$KAFKA_SSL_PORT"
fi
...
for VAR in `env`
do
  if [[ $VAR =~ ^KAFKA_ && ! $VAR =~ ^KAFKA_HOME && ! $VAR =~ ^KAFKA_PORT_ && ! $VAR =~ ^KAFKA_SERVICE_ ]]; then
